...
window.dp-theme-analog .dp-gauge-bg > viewport,
window.dp-theme-analog .dp-gauge-bg > * {
...
def _voltage_halfmoon_right(
    ...
    isz = max(12.0, r * 0.22)
    ix  = cx - r * 0.42 + isz
    iy  = cy + r * 0.28 + isz + 15  # 5 px lower
    bsz = isz * 1.35               # bolder / larger
    cr.set_source_rgba(*dim_col, 0.70 * a)
    cr.move_to(ix - bsz*0.18,  iy - bsz*0.80)
    cr.line_to(ix + bsz*0.18,  iy + bsz*0.06)
    cr.line_to(ix - bsz*0.08,  iy + bsz*0.06)
    cr.line_to(ix + bsz*0.18,  iy + bsz*0.80)
    cr.line_to(ix - bsz*0.18,  iy - bsz*0.06)
    cr.line_to(ix + bsz*0.08,  iy - bsz*0.06)
    cr.close_path()
    cr.fill()
...
def _draw_impl(cr: Any, width: int, height: int, data: Any, dark: bool) -> None:
    ...
    cr.paint()
    if width >= height:
        _draw_analog_landscape(cr, width, height, data, dark=dark)
    else:
        _draw_analog_portrait(cr, width, height, data, dark=dark)

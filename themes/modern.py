...
def _speed_ring(cr: Any, cx: float, cy: float, r: float, d: Any) -> None:
    ...
    grad_bg.add_color_stop_rgba(0.0, 0.13, 0.14, 0.18, 1.0)
    grad_bg.add_color_stop_rgba(1.0, 0.03, 0.04, 0.05, 1.0)
    ...
    cr.set_line_cap(1)
    cr.set_source_rgba(0.10, 0.12, 0.16, 0.90)
    ...
    for s in range(51):
        ...
        cr.set_line_cap(0)
        cr.set_source_rgba(0.82, 0.85, 0.90, (0.80 if major else 0.38) * a)
    ...
    for i in range(n_steps + 1):
        ...
        cr.set_source_rgba(*_DIM, 0.80 * a)
        cr.move_to(nx, ny)
        cr.show_text(txt)
    ...
    grad_face = cairo.RadialGradient(cx, cy - face_r * 0.28, face_r * 0.04,
                                     cx, cy, face_r)
    grad_face.add_color_stop_rgba(0.0, 0.16, 0.17, 0.20, 1.0)
    grad_face.add_color_stop_rgba(1.0, 0.05, 0.06, 0.08, 1.0)
    cr.set_source(grad_face)
    cr.arc(cx, cy, face_r, 0, math.tau)
    cr.fill()

    cr.set_line_width(max(1.2, r * 0.014))
    cr.set_source_rgba(0.30, 0.34, 0.40, 0.50)
    cr.arc(cx, cy, face_r, 0, math.tau)
    cr.stroke()
    ...
    if d.speed_source:
        ...
        _txt(cr, d.speed_source, cx, cy - r * 0.33, src_sz, (*_CYAN, 0.70 * a))

    # Speed value
    val_sz = max(24.0, r * 0.42)
    _txt(cr, d.speed_label, cx, cy - r * 0.02, val_sz,
         (*_WHITE, a), bold=True, max_w=face_r * 1.72)

    # Unit
    unit_sz = max(11.0, r * 0.160)
    _txt(cr, d.speed_unit, cx, cy + r * 0.26, unit_sz, (*_CYAN, 0.88 * a))


# ── Info column ───────────────────────────────────────────────────────────────

def _info_col(cr: Any, x: float, y: float, w: float, h: float, d: Any) -> None:
...
def _draw_modern_landscape(cr: Any, width: int, height: int, d: Any) -> None:
    ring_share = 0.60
    ...
    _speed_ring(cr, cx, cy, r, d)

    pad = height * 0.06
    _info_col(cr, ring_w, pad, info_w - pad * 0.5, height - pad * 2, d)


# ── Portrait layout ───────────────────────────────────────────────────────────

def _draw_modern_portrait(cr: Any, width: int, height: int, d: Any) -> None:
    ...
    _info_col(cr, pad, info_y, width - 2 * pad, info_h, d)


# ── Entry point ───────────────────────────────────────────────────────────────

def draw(cr: Any, width: int, height: int, data: Any) -> None:
    cr.set_source_rgb(0.036, 0.036, 0.052)
    cr.paint()
    strip_h = max(28.0, height * 0.072)
    if width >= height:
        _draw_modern_landscape(cr, width, height - strip_h, data)
    else:
        _draw_modern_portrait(cr, width, height - strip_h, data)

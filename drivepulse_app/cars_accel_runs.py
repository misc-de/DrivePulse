"""Acceleration-run list and detail helpers for CarsPage."""
...
class CarsAccelRunsMixin:
    def _render_accel_runs_into_value_list(self) -> None:
        ...
        try:
            ...
            log.exception("Could not list acceleration runs for car id=%s", self._selected_car_id)
            runs = []
        if not runs:
            self.value_list.append(self._info_row(_translate(self.language, "cars.accel_runs.empty")))
            return
    ...
    def _make_accel_run_row(self, run: Any) -> Adw.ActionRow:
        ...
        try:
            ...
            log.exception("Could not load acceleration run summary id=%s", run_id)
    ...
    def _open_accel_run_detail(self, run_id: int) -> None:
        ...
        try:
            ...
            log.exception("Could not open acceleration run detail id=%s", run_id)
            return
        if not data:
            return
        ...
        self._accel_run_detail_page = page
